...
@section('content')
    <div class="table-container">
        ...
        @if ($order->status === 'shipping')
            <button type="button" class="button-action" onclick="confirmPayment()">Xác nhận thanh toán</button>

            <form id="confirm-payment-form" action="{{ route('admin.order.confirmPayment', $order->id) }}" method="POST" style="display: none;">
                @csrf
                @method('PUT')
            </form>
        @endif
    ...
    <script>
        function confirmPayment() {
            if (confirm('Bạn có chắc chắn muốn xác nhận thanh toán cho đơn hàng này không?')) {
