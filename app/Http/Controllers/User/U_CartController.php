...
class U_CartController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Tính tổng số lượng và giá tiền sau giảm giá
        $totalQuantity = $cartItems->sum('quantity');
        $totalPrice = $cartItems->reduce(function ($carry, $cartItem) {
            if ($cartItem->type == 'product' && $cartItem->product) {
                $finalPrice = $cartItem->product->price * (1 - $cartItem->product->promotion / 100);
                return $carry + ($finalPrice * $cartItem->quantity);
            } elseif ($cartItem->type == 'ticket' && $cartItem->ticket) {
                $finalPrice = $cartItem->ticket->price * (1 - $cartItem->ticket->promotion / 100);
                return $carry + ($finalPrice * $cartItem->quantity);
            }
            return $carry; // Nếu không có product hoặc ticket, bỏ qua
        }, 0);

        // Xử lý thêm giá sau giảm giá trong từng mục cart_items
        $cartItems = $cartItems->map(function ($cartItem) {
            if ($cartItem->type == 'product' && $cartItem->product) {
                $cartItem->product->final_price = $cartItem->product->price * (1 - $cartItem->product->promotion / 100);
            } elseif ($cartItem->type == 'ticket' && $cartItem->ticket) {
                $cartItem->ticket->final_price = $cartItem->ticket->price * (1 - $cartItem->ticket->promotion / 100);
            }
            return $cartItem;
        });
        ...
        return response()->json([
            ...
            'total_price' => round($totalPrice, 2), // Tổng giá sau giảm giá
            'cart_items' => $cartItems, // Dữ liệu có trường giá sau giảm
        ]);
    }
