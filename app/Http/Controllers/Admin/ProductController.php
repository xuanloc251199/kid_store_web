...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $product = Product::create($request->only('name', 'category_id', 'detail', 'description', 'price', 'sold', 'quantity', 'promotion') + ['thumbnail' => $thumbnailPath]);;
        if (request()->is('api/*')) {
    ...
    public function update(Request $request, string $id)
    {
        ...
        $product->update($request->only('name', 'category_id', 'detail', 'description', 'price', 'sold', 'quantity', 'promotion') + ['thumbnail' => $thumbnailPath]);
        if (request()->is('api/*')) {
    ...
    {
        $request->validate([
            'promotion' => 'required|integer|min:0|max:100',
        ]);

        Product::query()->update(['promotion' => $request->input('promotion')]);

        return redirect()->route('admin.product.index')->with('success', 'Khuyến mãi đã được áp dụng cho tất cả sản phẩm!');
    }
}
